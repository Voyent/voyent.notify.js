    ...
    function _processMail(mail) {
        ...
        for (var famId in byFamilyId) {
            ...
            for (var k=0; k<notifications.length; k++) {
                // Change the alert state of the zone notification to match the alert state of
                // the most recent revision of the alert. If this is not available then that
                // notification is for the most recent revision so just use the existing state.
                notifications[k].alertState = notifications[k].alertFamilyState || notifications[k].alertState;
