...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            ...
            config: {
                ...
                clickListener:null, //expose custom click listener
                title:'',
                icon:'',
                ...
                toast: {
                    ...
                    hideAfterMs:5000, //milliseconds that the notification will be automatically hidden after shown (specify <=0 to never hide the notification)
                    stackLimit:3, //indicates the number of notifications that will stack (specify <=0 to have no limit)
                    overwriteOld:false, //indicates if new toast notifications should overwrite old ones
    ...
    function displayToast(toast) {
        ...
        if (notify.config.toast.hideAfterMs > 0) {
            _hideToastMessage(toast,notify.config.toast.hideAfterMs);
        }
    ...
    function _doOnLoad() {
        ...
        function onload() {
            ...
            //get unread mail from the mailbox service
            var params = window.location.search;
            var begIndexMailId = params.indexOf(MAIL_PARAMETER);
    
            if (begIndexMailId > -1) {
                var id = new RegExp( '[?&]' + MAIL_PARAMETER + '=([^&#]*)', 'i' ).exec(params);
                if (id) {
                    id = id[1];
                }
                ...
                if (window.history && history.pushState) {
                    ...
                    if (params === '?') {
                        ...
                        //remove trailing '&' character (if we removed from the end of the string)
                        if (params.charAt(params.length-1) === '&') {
                            params = params.slice(0,params.length-1);
                        }
                    }
                    //form the url with the modified query params
                ...
                v.io.mailbox.findMail({"id":v.io.auth.getLastKnownUsername(),"query":{"id":id}}).then(function(mail) {
                    if (mail && mail[0]) {
                        ...
                        //update queue and internal map
                        notify.queue.push(notification);
                        //update queue in session storage
                        _setQueueInStorage();
                        //set the current notification to the one retrieved
    ...
    function _setCurrentNotificationInStorage() {
        ...
        if (!notify.payload && !notify.metadata) {
            ...
            v.setSessionStorageItem(btoa(INJECT_KEY),btoa(JSON.stringify(notification)));
        }
        _fireEvent('afterCurrentNotificationSet',{'notification':notification},false,false);
    }
