...
(function (v) {
        ...
        notify = v.notify = {
            ...
            initialize: function() {
                if (!voyent.auth.isLoggedIn()) {
                    if (window.location.pathname.indexOf('vrasm') === -1) {
    ...
    function _setupListeners() {
        ...
        if (!_isInitialized) {
            ...
            //the VRASM app will process the query parameters internally so we can safely skip this step if using that app
            if (window.location.pathname.indexOf('vrasm') === -1) {
