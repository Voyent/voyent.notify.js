...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            /**
             * @property {Object} payload - The selected notification payload (readonly).
             * @default null
             */
            _payload: null,
            ...
            /**
             * @property {Object} payload - The selected notification metadata (readonly).
             * @default null
             */
            _metadata: null,
            ...
            config: { //config options
                ...
                /**
                 * @property {string} icon - The URL of the image to use as an icon for toast and browser native notifications.
                 * The recommended image size is 40px x 40px. The image will be scaled down to a width of 40px if it is larger.
                 * @default ''
                 */
                _icon:'',
            ...
            /**
             * Fired after a notification is selected. Not cancelable.
             * @event notificationSelected
             */
             ...
             * user is logged in via bridgeit.io. This function only needs to be called manually after stopListening has 
             * been called.
             */
            startListening: function() {
                ...
                this._listener = function (notification) {
                    ...
                    if (!_this.metadata) {
                        //if the metadata is not set then it means we don't have a selected
                        //notification so set this notification to this new one.
            ...
            removeNotificationAt: function(index) {
                ...
                if (notification) {
                    ...
                    if (notification.metadata.id) {
                        _removeNotificationFromMailbox(notification.metadata.id);
                    }
                    this._queue.splice(index,1);
                    _setQueueInStorage();
                    _fireEvent('afterQueueUpdated',{"op":"del","notification":notification,"queue":this.queue.slice(0)},false);
            ...
            removeSelectedNotification: function() {
                ...
                _fireEvent('notificationSelected',{"notification":this.getSelectedNotification()},false);
            ...
            /**
             * Redirects the browser to the URL specified in the metadata of the passed notification and injects the
             * notification data into the page.
            ...
            /**
             * Injects the selected notification into elements with data-payload-* and data-metadata-* attributes.
        ...
        function onload() {
            ...
            try {
                ...
                if (notification) {
                    ...
                    _fireEvent('notificationSelected',{"notification":notify.getSelectedNotification()},false);
    ...
    function _setSelectedNotificationInStorage() {
        if (!notify.metadata) {
            ...
            v.setSessionStorageItem(btoa(VOYENT_INJECT_KEY),btoa(JSON.stringify(notify.getSelectedNotification())));
        }
    }
