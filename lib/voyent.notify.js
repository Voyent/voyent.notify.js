...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            ...
            config: { //config options
                ...
                /**
                 * @property {String} group - The group used for push notifications.
                 * @default 'general'
                 */
                _group: 'general',
                set group(val) {
                    var oldGroup = this._group;
                    if (typeof val === 'string') { this._group = val; }
                    else { this.group = 'general'; }
                    if (oldGroup !== this._group) {
                        if (this._listener) {
            ...
            startListening: function() {
                if (this._listener || !this.config.group) { return; }
    ...
    /**
     * Adds a push listener for the currently set group and starts the push service if necessary.
     * @private
     */
    function _addPushListener() {
        //First try to add the listener and if it fails with a specific
        //error then start the push service and try again.
        v.push.addPushListener({
            'group': notify.config.group,
