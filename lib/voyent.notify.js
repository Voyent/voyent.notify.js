...
(function (v) {
    ...
    const VOYENT_QUEUE_KEY = 'voyentNotificationQueue';
    const VOYENT_TOAST_CONTAINER_ID = 'voyent_toast_container';
    const VOYENT_MAIL_PARAMETER = 'mailId';
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            /**
             * @property {Object} payload - The current notification payload (readonly).
             * @default null
             */
            _payload: null,
            ...
            /**
             * @property {Object} payload - The current notification metadata (readonly).
             * @default null
             */
            _metadata: null,
            ...
            /**
             * @property {Object[]} payload - The notification queue (readonly).
             * @default []
             */
            _queue: [],
            ...
            config: { //config options
                /**
                 * @property {boolean} hideAfterClick - Indicates if the notification should be hidden after clicking on it.
                 * @default true
                 */
                ...
                /**
                 * @property {boolean} title - Title/header text for each notification.
                 * @default ''
                 */
                _title:'',
                set title(val) { this._title = val.toString(); },
                get title() { return this._title; },

                /**
                 * @property {string} icon - The icon image to use for Toast and Native notifications. Max-width of 40px.
                 * @default ''
                 */
                _icon:'',
                set icon(val) { this._icon = val.toString(); },
                ...
                toast: { //toast notification config options
                    ...
                    /**
                     * @property {Number} hideAfterMs - Time in milliseconds that the notification will be automatically hidden after shown (specify <=0 to never hide the notification).
                     * @default 5000
                     */
                    _hideAfterMs: 5000,
                    set hideAfterMs(val) {
                    ...
                    /**
                     * @property {Number} stackLimit - Indicates the number of notifications that will be allowed to stack (specify <=0 to have no limit).
                     * @default 3
                     */
                    _stackLimit: 3,
                    set stackLimit(val) {
                    ...
                    /**
                     * @property {boolean} overwriteOld - Indicates if new toast notifications should overwrite/replace old ones in the stack.
                     * @default false
                     */
                    _overwriteOld: false,
                    ...
                    /**
                     * @property {string} position - Position of toast notifications on page. One of [top-right|top-left|bottom-right|bottom-left].
                     * @default 'top-right'
                     */
                    _position:'top-right',
                    set position(val) {
                    ...
                    /**
                     * @property {string} style - Custom styling for the toast notification container.
                     * @default ''
                     */
                    _style:'',
                    ...
                    close: { //toast notification close container config options
                        ...
                        /**
                         * @property {string} style - Custom styling for the toast notification close container.
                         * @default ''
                         */
                        _style:'',
                        set style(val) { this._style = val.toString(); },
                ...
                native: { //native/desktop notification config options
                    /**
                     * @property {boolean} enabled - Indicates if native notifications should be enabled (still must be allowed by user in browser).
                     * @default true
                     */
                    _enabled: true,
                    set enabled(val) { this._enabled = !!val; },
            ...
            /**
             * Fired after the library is initialized and listening for new notifications. Not cancelable.
            ...
            /**
             * Fired before a notification is displayed. Fires for both Toast and Native notifications.
            ...
            /**
             * Fired after a notification is displayed. Fires for both Toast and Native notifications. Not cancelable.
             * @event afterDisplayNotification
             */

            //Useful to do custom handling on the current notification. Especially after the asynchronous call of fetching a notification from the mailbox service.
            /**
             * Fired after the current notification is set (the current notification is the one returned
             * via voyent.notify[metadata/payload]). Not cancelable.
             * @event currentNotificationSet
             */

            //Useful for custom redirecting (for apps that use custom routing).
            /**
             * Fired when a notification is clicked. Fires for both Toast and Native notifications. Cancel the
             * event to prevent the app from redirecting and the notification from closing (if hideAfterClick is true).
             * @event notificationClicked
             */

            //Useful for custom close behaviour or preventing the notification from closing.
            /**
             * Fired when a notification is closed. Fires for both Toast and Native notifications. Cancel the event
            ...
            _queuePos: -1, //the position of the current notification in the queue
            ...
            startListening: function() {
                ...
                this._listener = function (notification) {
                    ...
                    _setQueueInStorage();
                    _fireEvent('afterQueueUpdated',{"queue":_this.queue.slice(0)},false);
                    _displayNotification(notification);
                    if (!_this.metadata) {
                        //if the metadata is not set then it means we don't have a "current"
                        //notification so set this notification to the current one.
                        _this.setCurrentNotification(notification);
                        _this._queuePos = 0;
                        //inject notification data into the page if they are on
                        //the relevant page and currently have no active notification
                ...
                _fireEvent('voyentNotifyInitialized',{"config":_this.config},false);
            },
            ...
            stopListening: function() {
                v.xio.push.removeListener();
                this._listener = null;
                //since they explicitly stopped listening them remove the login listener as well
                window.removeEventListener('bridgeit-login-succeeded',_setupListeners);
            },
            ...
            /**
             * Return the currently active notification or null if there is no current notification.
             * @returns {*}
             */
            getCurrentNotification: function() {
                return this.metadata ? {"payload":this.payload,"metadata":this.metadata} : null;
            },
             ...
             * @param {Object} notification - The notification object to be removed.
            ...
            removeNotification: function(notification) {
                ...
                if (index > -1) {
                    ...
                    if (this.queue[index].metadata.id) {
                        _removeNotificationFromMailbox(this.queue[index].metadata.id);
                    }
                    this._queue.splice(index,1);
                    _setQueueInStorage();
                    _fireEvent('afterQueueUpdated',{"queue":this.queue.slice(0)},false);
                    return true;
                }
                return false;
            },
            ...
            removeNotificationAt: function(index) {
                if (this.queue[index]) {
                    var cancelled = _fireEvent('beforeQueueUpdated',{"op":"del","notification":this.queue[index],"queue":this.queue.slice(0)},true);
                    if (cancelled) {
                       return false;
                    }
                    //if we have an id it means the notification is stored in the
            ...
            removeCurrentNotification: function() {
                ...
                var cancelled = _fireEvent('beforeQueueUpdated',{"op":"del","notification":this.queue[this._queuePos],"queue":this.queue.slice(0)},true);
                if (cancelled) {
                    return false;
                }
                //if we have an id it means the notification is stored in the
                //mailbox service so we will delete it from the user's mail
                if (this.metadata.id) {
                    _removeNotificationFromMailbox(this.metadata.id);
                }
                this._payload = null;
                this._metadata = null;
                _setCurrentNotificationInStorage();
                _fireEvent('currentNotificationSet',{"notification":this.getCurrentNotification()},false);
                this._queue.splice(this._queuePos,1);
                _setQueueInStorage();
                this._queuePos = -1;
                _fireEvent('afterQueueUpdated',{"queue":this.queue.slice(0)},false);
                return true;
            ...
            clearNotificationQueue: function() {
                ...
                for (var i=0; i<this.queue.length; i++) {
                    if (this.queue[i].metadata.id) {
                        _removeNotificationFromMailbox(this.queue[i].metadata.id);
                    }
                }
                this._queue = [];
                _setQueueInStorage();
            ...
            /**
             * Redirects the browser to the URL specified in the metadata of the passed
             * notification and after load injects the notification data into the page.
            ...
            setCurrentNotification: function(notification) {
                var index = this.queue.indexOf(notification);
                if (index > -1) {
                    this._payload = notification.payload ? notification.payload : null;
                    this._metadata = notification.metadata;
                    this._queuePos = index;
                    _setCurrentNotificationInStorage();
                    _fireEvent('currentNotificationSet',{"notification":this.getCurrentNotification()},false);
                    return true;
                }
            ...
            /**
             * Manually hides the passed in notification reference.
             * @param {Object} notificationRef - A reference to a Toast or Native notification (references should be retrieved from event data).
             * @param {number} ms - The time in milliseconds that the notification will be hidden.
             */
            hideNotification: function(notificationRef,ms) {
                if (!notificationRef || !notificationRef.constructor) {
                    return;
                }
                setTimeout(function() {
                    if (notificationRef.constructor === HTMLDivElement) { //toast notification
                        ...
                        notificationRef.style.transform = 'translateY('+hideTranslateY+'px)';
                        notificationRef.style.webkitTransform = 'translateY('+hideTranslateY+'px)';
                        setTimeout(function() {
                            if (document.getElementById(VOYENT_TOAST_CONTAINER_ID).contains(notificationRef)) {
                                document.getElementById(VOYENT_TOAST_CONTAINER_ID).removeChild(notificationRef);
                                _updateDisplayedNotifications(notificationRef);
                            }
                        },400); //transition effect is for 300ms so remove the toast from the DOM after 400ms
                    }
                    else if (notificationRef.constructor === Notification) { //native notification
    ...
    const TOAST_Y_POS = 100; //the starting vertical position of toast notifications from the top or bottom of the page
    ...
    function _initialize() {
        if (!bridgeit.io.auth.isLoggedIn()) {
            window.addEventListener('bridgeit-login-succeeded',_setupListeners);
        }
        else {
            _setupListeners();
        }
    ...
    function _parseNotification(notification) {
        try {
            //move the payload and metadata out of the message property
            notification.payload = notification.message.payload ? notification.message.payload : null;
            notification.metadata = notification.message.metadata;
            delete notification.message;
            //move other properties into the metadata
            ...
            notification.metadata.id = notification.id; //this will only be present if the notification was stored in the mailbox service
    ...
    /**
     * Determines if we should display a Toast or Native notification.
    ...
    function _displayNativeNotification(notification) {
        ...
        if (notify.config.icon.length > 0) {
            opts.icon = notify.config.icon;
        }
        //display the notification
        var n = new Notification(notify.config.title,opts);
        //add onclick listener with default behaviour of redirecting and closing the notification
        n.onclick = function() {
    ...
    /**
     * Searches for relevant DOM elements that notification payload and metadata should be injected or cleared on.
    ...
    function _injectOrClearNotficationData (doClear) {
        ...
        for (key in notify.payload) {
            ...
            for (i=0; i<elements.length; i++) {
                _injectOrClearDataForType(elements[i],notify.payload[key],doClear);
            }
        }
        //metadata injection / clearing
        for (key in notify.metadata) {
            ...
            for (i=0; i<elements.length; i++) {
                _injectOrClearDataForType(elements[i],notify.metadata[key],doClear);
            }
        }
    }
        ...
        switch (element.tagName) {
            ...
            case 'SELECT':
                ...
                for (var i=0; i<data.length; i++) {
                    ...
                    opt.value = data[i].value ? data[i].value : data[i];
                    opt.textContent = data[i].label ? data[i].label : data[i];
        ...
        function onload() {
            ...
            try {
                ...
                if (queue && queue.length > 0) {
                    ...
                    notify._queue = queue;
                    _fireEvent('afterQueueUpdated',{"queue":notify.queue.slice(0)},false);
                }
            }
            catch (err) {
            ...
            try {
                ...
                if (notification) {
                    //set the current notification and inject data
                    notify.setCurrentNotification(notification);
                    _fireEvent('currentNotificationSet',{"notification":notify.getCurrentNotification()},false);
                    notify.injectNotificationData();
                }
            }
            catch(err) {
                console.log('Error loading current notification from storage',err);
            }

            //get unread notifications from the mailbox service
            var params = window.location.search;
            var begIndexMailId = params.indexOf(VOYENT_MAIL_PARAMETER);
            if (begIndexMailId > -1) {
                var id = new RegExp( '[?&]' + VOYENT_MAIL_PARAMETER + '=([^&#]*)', 'i' ).exec(params);
                if (!id) {
                    return;
                }
                id = id[1];
                ...
                if (window.history && history.pushState) {
                    //get the mailId parameter and any trailing parameters
                ...
                v.io.mailbox.findMail({"id":v.io.auth.getLastKnownUsername(),"query":{"id":id}}).then(function(mail) {
                    if (mail && mail[0]) {
    ...
    function _setQueueInStorage() {
        if (!notify.queue || !notify.queue.length) {
            v.removeSessionStorageItem(btoa(VOYENT_QUEUE_KEY));
        }
        else {
            v.setSessionStorageItem(btoa(VOYENT_QUEUE_KEY),btoa(JSON.stringify(notify.queue)));
        }
    }
    ...
    function _getQueueFromStorage() {
        return JSON.parse(atob(v.getSessionStorageItem(btoa(VOYENT_QUEUE_KEY))));
    }

    /**
     * Stores the current notification in session storage.
     * @private
     */
    function _setCurrentNotificationInStorage() {
        if (!notify.metadata) {
            v.removeSessionStorageItem(btoa(VOYENT_INJECT_KEY));
        }
        else {
            v.setSessionStorageItem(btoa(VOYENT_INJECT_KEY),btoa(JSON.stringify(notify.getCurrentNotification())));
        }
    }

    /**
     * Retrieves the current notification from session storage.
     * @returns {Object} - The current notification.
     * @private
     */
    function _getCurrentNotificationFromStorage() {
        return JSON.parse(atob(v.getSessionStorageItem(btoa(VOYENT_INJECT_KEY))));
    }
