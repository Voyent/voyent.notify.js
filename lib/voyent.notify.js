...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            ...
            /**
             * Fetches all the notifications in the current user's mailbox. If notifications are found then the queue
            ...
            refreshNotificationQueue: function(nid) {
                ...
                if (v.auth.isLoggedIn()) {
                    v.mailbox.findMessages({
                        "username":v.auth.getLastKnownUsername(),
                        "fields":{"_id":0},
                        "options":{"sort":{"time":-1}}
                    }).then(function(mail) {
                        if (mail) {
                            mail = _this._processMail(mail);
            ...
            /**
             * Ensures notifications for revised alerts still show up under the `active` or `ended` tabs in the VRAS
             * application. We do this by always displaying 1 notification for each alert zone and changing the alert
             * state of that notification to match the alert state of the most recent alert revision.
             * http://jira.icesoft.org/browse/VRAS-546
             * @param mail
             * @private
             */
            _processMail: function(mail) {
                // Create an object where the properties represent "{{alertFamilyId}}:[notificationsForFamilyId]
                var byFamilyId = {};

                for (var i=0; i<mail.length; i++) {
                    var alertFamilyId = mail[i].alertFamilyId;
                    if (!byFamilyId[alertFamilyId]) {
                        byFamilyId[alertFamilyId] = [];
                    }
                    // Since the mail list comes in sorted this will create a sorted list of
                    // notifications for a specific alertFamilyId (all revisions of an alert)
                    byFamilyId[alertFamilyId].push(mail[i]);
                }
                mail = [];

                for (var famId in byFamilyId) {
                    if (!byFamilyId.hasOwnProperty(famId)) {
                        continue;
                    }
                    // Grab our list of notifications for a particular alertFamilyId
                    var notifications = byFamilyId[famId];
                    // The current alert state is reflected in the most recent
                    // notification received for a specific alertFamilyId
                    var currentAlertState = notifications[0].alertState;
                    // A list of zones which we already have a notification for
                    var processedZoneList = [];

                    for (var k=0; k<notifications.length; k++) {
                        // Change the alert state of the zone notification to match the alert state of the
                        // most recent revision of the alert. This alert state will be `active` or `ended`
                        notifications[k].alertState = currentAlertState;
                        var zoneId = notifications[k].affectedLocations && notifications[k].affectedLocations[0] ?
                            notifications[k].affectedLocations[0].properties.vras.insideZoneId :
                            'unknown';

                        // Since the notifications are sorted we only need to keep
                        // the first (most recent) notification for each zone
                        if (processedZoneList.indexOf(zoneId) > -1 && zoneId !== 'unknown') {
                            continue;
                        }
                        // Push the notificaiton to our final list
                        mail.push(notifications[k]);
                        // Indicate that we already have a notification for this zoneId
                        processedZoneList.push(zoneId);
                    }
                }
                return mail;
            },
