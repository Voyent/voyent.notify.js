...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            ...
            startListening: function() {
                ...
                // Note we remove any periods (.) or dollar signs ($) to prevent errors from the URLs
                var groupToJoin = v.auth.getLastKnownUsername();
                groupToJoin = groupToJoin.replace(/\./g, "_");
                groupToJoin = groupToJoin.replace(/\$/g, "_");
    ...
    function _setupListeners() {
        ...
        if (!_isInitialized) {
            ...
            try {
                ...
                _fireEvent('message-error','Error loading queue from storage: ' +
                          (e.responseText || e.message || e), false);
            }
            ...
            if (begIndexMailId > -1) {
                ...
                //Fetch all notifications from the user's mailbox, add them
                //to the queue and select the one identified by the nid.
                v.mailbox.findMessages({"username":v.auth.getLastKnownUsername(),"fields":{"_id":0}}).then(function(mail) {
                    if (mail) {
                        ...
                        for (var i=0; i<mail.length; i++) {
                            ...
                            if ((existingNotification && existingNotification.nid === nid) ||
    ...
    function _parseNotification(notification) {
        try {
            ...
            for (var key in notification.push_configuration) {
                if (!notification.push_configuration.hasOwnProperty(key)) {
                    continue;
                }
                notification[key] = notification.push_configuration[key];
            }
            delete notification.push_configuration;
        }
        catch (e) {
            return false;
        }
