...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
             ...
             * user is logged in via voyent.js. This function only needs to be called manually after stopListening has
             * been called.
             */
            startListening: function() {
                ...
                this.addPushListener(v.auth.getLastKnownUsername());
            },
            ...
            stopListening: function() {
                for (var i=0; i<this._groups.length; i++) {
                    this.removePushListener(this._groups[i]);
                }
                this._listener = null;
             ...
             * @param group
             */
            addPushListener: function(group) {
                if (group && typeof group === 'string' &&
                    this._groups.indexOf(group) === -1) {
                    _addPushListener(group);
                }
            },
             ...
             * @param group
             */
            removePushListener: function(group) {
                if (group && typeof group === 'string') {
                    var index = this._groups.indexOf(group);
                    if (index > -1) {
                        _removePushListener(group,index);
                    }
                }
            },
    ...
    /**
     * Setup the push listener and onload listener.
     * @private
     */
    ...
    /**
     * Adds a push listener for the passed group name and starts the push service if necessary.
     * @param {string} group - The group to add a new listener for.
     * @private
     */
    function _addPushListener(group) {
        //First try to add the listener and if it fails with a specific
        //error then start the push service and try again.
        v.push.addPushListener({
            'group': group,
            'callback': notify._listener
        }).then(function() {
            notify._groups.push(group);
        }).catch(function(error) {
            if (error === 'Push service is not active') {
                voyent.push.startPushService({}).then(function () {
                    _addPushListener(group);
                }).catch(function (e) {
                    console.error('Error when starting the push service: ', e)
                });
            }
        });
    }
    ...
    /**
     * Removes the push listener for the passed group name.
     * @param {string} group - The group to remove the listener for.
     * @param {number} index - The index to be removed from the groups array.
     * @private
     */
    function _removePushListener(group,index) {
        v.push.removePushListener({
            'group': group
        }).then(function() {
            notify._groups.splice(index,1);
        });
    }
    ...
    function _removeNotificationFromMailbox(id) {
        ...
        v.mailbox.deleteMessages({"username":v.auth.getLastKnownUsername(),
                                     "query":{"notificationId":id}}).then(function() {
        }).catch(function(error) {
            console.log('Error trying to delete notification',error);
        });
    }
    ...
    function _setupOnload() {
        ...
        function onload() {
            ...
            if (begIndexMailId > -1) {
                ...
                v.mailbox.findMessages({"username":v.auth.getLastKnownUsername(),"query":{"notificationId":notificationId}}).then(function(mail) {
                    if (mail && mail[0]) {
    ...
    function _setQueueInStorage() {
        var queueKey = VOYENT_QUEUE_KEY+'_'+v.auth.getLastKnownUsername();
