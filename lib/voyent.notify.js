...
(function (v) {
    ...
    if (!v.notify) {
        ...
        notify = v.notify = {
            ...
            removeSelectedNotification: function() {
                ...
                _fireEvent('notificationSelected',{"notification":this.selected},false);
    ...
    function _injectOrClearDataForType (element,data,doClear) {
        switch (element.tagName) {
            ...
            default:
                ...
                if (!doClear) {
                    //check if the incoming data is a valid date, in which case we
                    //wrap it in a Date object to ensure the timezone is local
                    try {
                        element.textContent = (!isNaN(Date.parse(data))) ? new Date(data) : data;
                    }
                    catch (e) {
        ...
        function onload() {
            ...
            try {
                ...
                if (notification) {
                    ...
                    _fireEvent('notificationSelected',{"notification":notify.selected},false);
    ...
    function _getQueueFromStorage() {
        var base64Queue = v.getSessionStorageItem(btoa(VOYENT_QUEUE_KEY));
    ...
    function _setSelectedNotificationInStorage() {
        if (!notify.selected) {
            ...
            v.setSessionStorageItem(btoa(VOYENT_INJECT_KEY),btoa(JSON.stringify(notify.selected)));
        }
    }
    ...
    function _getSelectedNotificationFromStorage() {
        var base64Notification = v.getSessionStorageItem(btoa(VOYENT_INJECT_KEY));
